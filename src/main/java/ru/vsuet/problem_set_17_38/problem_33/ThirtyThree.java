package ru.vsuet.problem_set_17_38.problem_33;
    ...
    private static long playGame(long sticks, boolean tanyaTurn) {
        ...
        if (sticks % 2 == 0) { // если количество палочек четное
            if (tanyaTurn) { // если ход Тани
                return playGame(sticks / 2, false); // Таня возьмет половину палочек
            } else { // если ход Саши
                if (sticks == 2) { // если осталось 2 палочки, то Саша возьмет 1
                    return 1;
                } else {
                    return Math.max(playGame(sticks - 1, true), playGame(sticks / 2, !tanyaTurn)); // Саша возьмет 1 или половину палочек, чтобы оставить меньшее количество палочек Тане
                }
            }
